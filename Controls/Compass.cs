...
namespace GeoFramework.Gps.Controls
{
	...
	public sealed class Compass : PolarControl
	{
		...
		public Compass()
			: base("GeoFramework Multithreaded Compass Control (http://www.geoframeworks.com)")
		{
#if !PocketPC
			...
			_InterpolationThread.Name = "GeoFramework Compass Needle Animation Thread (http://www.geoframeworks.com)";
		...
		public bool IsUsingRealTimeData
		{
			...
			set
			{
                ...
                if (pIsUsingRealTimeData)
                {
                    ...
                    Value = GeoFramework.Gps.IO.Devices.Bearing;
                }
                else
                {
                    // Only hook into events if we're at run-time.  Hooking events
