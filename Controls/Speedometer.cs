...
namespace GeoFramework.Gps.Controls
{
    ...
    public sealed class Speedometer : PolarControl
	{
		...
		public Speedometer()
			: base("GeoFramework Multithreaded Speedometer Control (http://www.geoframeworks.com)")
		{
#if !PocketPC
			...
			InterpolationThread.Name = "GeoFramework Speedometer Needle Animation Thread (http://www.geoframeworks.com)";
...
//            GeoFramework.Gps.IO.Devices.SpeedChanged += new EventHandler<SpeedEventArgs>(Devices_CurrentSpeedChanged);
//#endif
        ...
        public bool IsUsingRealTimeData
		{
			...
			set
			{
                ...
                if (pIsUsingRealTimeData)
                {
                    ...
                    Value = GeoFramework.Gps.IO.Devices.Speed;
                }
                else
                {
                    // Only hook into events if we're at run-time.  Hooking events
